...
- If the canonical_id is NOT listed:
  ...
  - Do NOT generate code.
  - Run `swytchcode add <canonical_id>`, then confirm it appears in `swytchcode list tooling`.
...
with filter `tooling`). If it is not listed, call swytchcode_add first. Never generate
